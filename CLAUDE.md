...
node my-first-harness.ts haiku        # or luna / fable / farm — selects adapter + connection
...
**Server-side web search** is enabled on every connection (`webSearch` in `model-config.ts`): the Anthropic adapter appends `{ type: "web_search_20250305" | "web_search_20260209", name: "web_search", max_uses: 5 }` after the function tools (Haiku 4.5 needs the older tool name, Fable the newer), the Responses adapter appends `{ type: "web_search" }`. The model decides when to search; the harness implements no search of its own. Provider-executed blocks (`server_tool_use`, any `*_tool_result` — Fable also returns `code_execution_tool_result`) and Responses `web_search_call` items are accepted, hidden from the common content, and kept in `replayState` (Anthropic) or dropped from replay (Responses). Anthropic's `pause_turn` (a paused long-running server tool) is still treated as an unknown stop reason and fails explicitly; its search count lands in `LLMUsage.webSearchRequests`. The `game-testing` skill routes the model to `strategy-tool.md` and `state-contract.md` in its folder instead of carrying everything in `SKILL.md`.
...
**Model-made tools** (`tools/custom-tools.ts`, plugin `custom-tools`): `createTool` saves JavaScript (`export default async function run(args)`) plus a JSON-schema and registers it immediately; `deleteTool` removes it. Tools are **session-only by default** (stored under the OS temp dir, removed when the plugin stops); `persist: true` stores them under `<workspace>/.my-first-harness/tools/<name>/` and re-registers them at startup with a console notice, so a stale strategy tool cannot silently leak into the next session. Each call runs `custom-tools/runner.mjs` in a child `node` process with only `PATH`/`HOME`/`LANG`/`TOOL_DIR`/`WORKSPACE_DIR` in its environment (no API keys), a timeout (30 s default), and a result marker so stray stdout cannot corrupt the result. Creating, deleting and running these tools are `ask` in `INTERACTIVE_PERMISSIONS`, like `runCommand`. This is why `PluginManager` lets an *active* plugin register tools after `setup` — late registrations are tracked and unwound on disable. The harness knows nothing about what the tools compute; the `tool-making` skill tells the model when and how to build one.

**Game state contract**: `gameTestObserve({ format: "text" | "both" })` reads `window.__gameTest.getState()` through the controller's `state` command and forwards the JSON verbatim; games without the contract get a guidance error, not a `lost` test. The harness never interprets the state.

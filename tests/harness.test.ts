...
const definitions = stripTypeScriptTypes([
  ...
  section("class SkillManager", "const skillManager"),
...
function harness(adapter: LLMAdapter) {
  ...
  const saved: any[] = [];
  const deps = {
    Ajv, adapter, ...context, summarize, textOf, randomUUID,
